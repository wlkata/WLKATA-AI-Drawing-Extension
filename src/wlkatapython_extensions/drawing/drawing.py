...
class Drawing:
    ...
    def edges_from_image(
        ...
        if isinstance(image_path, str):
            ...
            import numpy as np
            images = [cv2.imread(im, cv2.IMREAD_GRAYSCALE) for im in image_path]
            ...
            image = np.zeros((height, total_width), dtype=np.uint8)

            # Place each image side by side
            x_offset = 0
            for img in images:
                w = img.shape[1]
                image[:, x_offset:x_offset + w] = img
                x_offset += w
        else:
    ...
    def resize_point_groups(
        ...
        x_range = temp_max_x - temp_min_x + 1e-3
        y_range = temp_max_y - temp_min_y + 1e-3

        print(x_bound, y_bound, x_range, y_range)
    ...
    def draw_area(
        ...
        Returns:
            None
        """
        self.robot.sendMsg(f'M20 G90 G00 X{self.min_x} Y{self.min_y} Z{self.z + 20}')
    ...
    def pre_drawing(
        ...
        while (line := input('Sample plot looks good? (y/[n]): ')).lower() != 'y':
            if line == '' or line.lower() == 'n':
                # TODO: smoothness
                return False
        self.draw_area(z_offset=0 if draw_bound else 10)
...
if __name__ == "__main__":
    ...
    # drawer.draw(['sample/Slide1.png'])
    drawer.draw('sample/face8.png', binary_threshold=225, blur=True, rotation=2)
